...
const ForgotPassword = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${backend_URL}/api/player/send-forgotpassword-otp`, fetchOptions);
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${backend_URL}/api/player/verify-forgotpassword-otp`, fetchOptions);
      const data = await response.json();
      if (data.success) {
        toast.success(data.message || 'OTP verified');
        setTimeout(() => {
          navigate('/reset-password-player', { state: { email } });
        }, 40);
      } else {
  ...
  return (
    <div className="forgotpw-container">
      <div className="forgotpw-wrapper">
        <button className="forgotpw-back-button" onClick={() => navigate('/login/player')}>
